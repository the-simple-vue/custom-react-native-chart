...
export const drawXAxisLabels = (sortedData, gap, color = '#000000', showEvenNumberXaxisLabel) => {
  return (
    <View style={{
      ...
      {sortedData.filter(item=>item['x']!=='0').map((data, i) => {
        if (data['x'] && i % 2 === 1 || !showEvenNumberXaxisLabel) {
          return (
            <View key={'label' + i} style={{
              position: 'absolute',
              left: data['gap'] - gap / 2 + 6,
              width: gap - 7,
              alignItems: 'center',
              paddingBottom: 10
            }}>
              <Text style={{fontSize: 9, color: color,textAlign:'center',paddingBottom:10,paddingTop:5}} numberOfLines={2} ellipsizeMode="tail">
                {
                  data['x']
                }
              </Text>
