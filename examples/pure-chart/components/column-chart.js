...
import { drawGuideLine, drawXAxis, drawXAxisLabels, drawYAxis, drawYAxisLabels, initData, numberWithCommas } from '../common'
...
export default class ColumnChart extends Component {
  ...
  componentDidMount () {
    Animated.timing(this.state.fadeAnim, {
      toValue: 1, easing: Easing.bounce, duration: 0, useNativeDriver: true
    }).start()
  }
  ...
  drawTooltip (selectedIndex) {
    if (typeof (selectedIndex) === 'number' && selectedIndex >= 0) {
      ...
      if (this.state.sortedData.length === 1) {
        plusGap = 0
      } else if (selectedIndex === standardSeries.data.length === 0) {
        plusGap = -50
      }
      // 차트 width를 마지막에 늘려야겠음.
      let left = standardSeries.data[selectedIndex]['gap'] + plusGap
      let tooltipRenders = []
      let isRenderPossible=false
      for (let i = 0; i < this.state.sortedData.length; i++) {
        let series = this.state.sortedData[i]
        if (series.data[selectedIndex]['x']) {
          tooltipRenders.push(<Text key={'tooltipTitle-' + i} style={styles.tooltipTitle}>{series.seriesName}</Text>)
        }
        isRenderPossible= series.data[selectedIndex]['y']===0&&i===0
        tooltipRenders.push(
      ...
      return (
        <View style={[styles.tooltipWrapper, { left: left }]} pointerEvents="none">
    ...
    return (
      <View style={StyleSheet.flatten([styles.wrapper, {
        ...
        <View style={{paddingRight: 5}}>
          <Text style={{  bottom: 20,right:5,
            position: 'absolute',fontSize: 11,textAlign:'right'}}>0</Text>
          {drawYAxisLabels(this.state.guideArray, this.props.height + 20, this.props.minValue, this.props.labelColor)}
        </View>
        <View style={styles.mainContainer}>
          <ScrollView nestedScrollEnabled={true}>
            <ScrollView horizontal contentContainerStyle={{paddingRight:100}}>
